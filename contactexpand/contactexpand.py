...
def main():
  ...
  rowrow.pop(0) 
  countone = countone-1
  print(time.strftime('[%d/%m %H:%M:%S]',time.localtime())+' Contact list extracted.\n\nExpanding...')
  if countone>500: timetorest=0.05
  if countone>1000: timetorest=0
  time.sleep(timetorest) 
  for row in rowrow:
    rowrowrow.append(row[0:2])
  print(time.strftime('[%d/%m %H:%M:%S]',time.localtime())+' There are '+str(countone)+' records to expand, it may take some time.')
  time.sleep(timetorest) 
  ...
  for row in rowrowrow:
    for rowtwo in rowrow:
      ...
      if rownorow[-1][0]==rownorow[-1][2]:
        rownorow.pop()
  #print(time.strftime('[%d/%m %H:%M:%S]',time.localtime()))
  print(time.strftime('[%d/%m %H:%M:%S]',time.localtime())+' Expanded to memory ok. Now writing to output.csv '+str((countone*countone)-countone)+' records...')
  ...
  #counttwo= counttwo+1  
  print(time.strftime('[%d/%m %H:%M:%S]',time.localtime())+' Done expanding '+str(countone)+' records into '+str((countone*countone)-countone)+' records in ' + str(format(time.time()-timestart-(timetorest*2)-(timetorest/100)*((((countone*countone)-countone))/countone), '.4f') + ' seconds'))
