...
def createmultilogger(name):
    # One can put __name__ here for module's name to appear in logs
    multilog = logging.getLogger(name)
    # basic level DEBUG INFO WARNING ERROR etc
    multilog.setLevel(logging.DEBUG)
    # create file handler which logs even debug messages
    fh = logging.FileHandler(
        '/var/log/%s.log',
        name)  # we MUST have WRITE permissions on this file
    fh.setLevel(logging.DEBUG)
    # create console handler with a higher log level - you don't want to flood
    # your console
    ch = logging.StreamHandler()
    ch.setLevel(logging.INFO)
    # create formatter and add it to the handlers
    multilogformatter = logging.Formatter(
